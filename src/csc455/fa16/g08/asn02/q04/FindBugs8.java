...
public class FindBugs8 {
	public static void main(final String[] args) {
		System.out.println("Findbugs Sample 001 for BC_IMPOSSIBLE_CAST");
		// WRONG
		try {
			FindBugs8.bcImpossibleCastWRONG();
		} catch (final ClassCastException e) {
			System.out.println(" - ERROR:" + e.getMessage());
		}
		// CORRECT
		FindBugs8.bcImpossibleCastCORRECT();
		System.out.println("Findbugs Sample 002 for BC_IMPOSSIBLE_DOWNCAST");
		// WRONG
		try {
			FindBugs8.bcImpossibleDowncastWRONG();
		} catch (final ClassCastException e) {
			System.out.println(" - ERROR:" + e.getMessage());
		}
		// CORRECT
		FindBugs8.bcImpossibleDowncastCORRECT();
		System.out.println("Findbugs Sample 003 for BC_IMPOSSIBLE_INSTANCEOF");
		// WRONG
		FindBugs8.bcImpossibleInstanceOfWRONG();
		// CORRECT
		FindBugs8.bcImpossibleInstanceOfCORRECT();
		System.out.println("Findbugs Sample 004 for BC_IMPOSSIBLE_DOWNCAST_OF_TOARRAY");
		// WRONG
		try {
			FindBugs8.bcImpossibleDowncastOfArrayWRONG();
		} catch (final ClassCastException e) {
			System.out.println(" - ERROR:" + e.getMessage());
		}
		// CORRECT
		FindBugs8.bcImpossibleDowncastOfArrayCORRECT();
		System.out.println("Findbugs Sample 005 for DMI_BIGDECIMAL_CONSTRUCTED_FROM_DOUBLE");
		// WRONG
		FindBugs8.dmiBigDecimalConstructedFromDoubleWRONG();
		// CORRECT
		FindBugs8.dmiBigDecimalConstructedFromDoubleCORRECT();
		System.out.println("Findbugs Sample 006 for ES_COMPARING_STRINGS_WITH_EQ");
		// WRONG
		FindBugs8.esComparingStringsWithEqWRONG();
		// CORRECT
		FindBugs8.esComparingStringsWithEqCORRECT();
		System.out.println("Findbugs Sample 007 for VA_FORMAT_STRING_ILLEGAL");
		// WRONG
		try {
			FindBugs8.vaFormatStringIllegalWRONG();
		} catch (final UnknownFormatConversionException e) {
			System.out.println(" - ERROR:" + e.getMessage());
		}
		// CORRECT
		FindBugs8.vaFormatStringIllegalCORRECT();
		System.out.println("Findbugs Sample 008 for RV_RETURN_VALUE_IGNORED");
		// WRONG
		FindBugs8.rvReturnValueIgnoredWRONG();
		// CORRECT
		FindBugs8.rvReturnValueIgnoredCORRECT();
		System.out.println("Findbugs Sample 009 for NP_ALWAYS_NULL");
		// WRONG
		try {
			FindBugs8.npAlwaysNullWRONG();
		} catch (final NullPointerException e) {
			System.out.println(" - ERROR:" + e.getMessage());
		}
		// CORRECT
		FindBugs8.npAlwaysNullCORRECT();
		System.out.println("Findbugs Sample 010 for QBA_QUESTIONABLE_BOOLEAN_ASSIGNMENT");
		// WRONG
		FindBugs8.qabQuestionableBooleanAssignmentWRONG();
		// CORRECT
		FindBugs8.qabQuestionableBooleanAssignmentCORRECT();
	}
